...
export const S06ResponseLeg: React.FC = () => {
  ...
  return (
    <SceneBg>
      ...
      {/* endpoint nodes */}
      <div style={{ position: "absolute", top: 330, left: 150, opacity: 0.9 * cardVisible }}>
        <div style={{ background: C.white, border: `1px solid ${C.borderSoft}`, borderRadius: 16, padding: "22px 34px", fontFamily: FONT, fontSize: 26, fontWeight: 500, color: C.ink }}>
          AGENT RUNTIME
          <div style={{ fontFamily: FONT, fontSize: 18, fontWeight: 500, color: C.gray, marginTop: 4 }}>executes the action</div>
        </div>
      </div>
      <div style={{ position: "absolute", top: 330, right: 150, opacity: 0.9 * cardVisible }}>
        <div style={{ background: C.inverse, borderRadius: 16, padding: "22px 34px", fontFamily: FONT, fontSize: 26, fontWeight: 500, color: C.white, textAlign: "center" }}>
          Holo3-35B
      ...
      {/* wire */}
      <div style={{ position: "absolute", top: 386, left: 420, right: 420, borderTop: `3px solid ${C.borderSoft}`, opacity: cardVisible }} />
      ...
      >
        ...
        >
          ...
          <span style={{ color: C.green, fontWeight: 500 }}>"«{TOKEN}»"</span>)
        </div>
      </div>
        ...
        >
          <div
            style={{
              ...
              background: C.white,
              padding: "58px 90px",
              boxShadow: `0 16px 48px rgba(12,12,12,.06)${settled ? `, 0 0 0 1.5px ${C.green}` : ""}`,
              textAlign: "center",
            }}
          >
            <div style={{ ...T.label, color: C.gray, marginBottom: 26 }}>inside the proxy · resolving against the vault</div>
            <div style={{ fontSize: 58, fontWeight: 500 }}>
            ...
            <div style={{ display: "flex", gap: 40, justifyContent: "center", marginTop: 34, fontFamily: MONO, fontSize: 21, color: C.gray }}>
              <span>
                vault: <span style={{ color: C.ink }}>{TOKEN}</span> →{" "}
                <span style={{ color: C.grayLight, letterSpacing: "0.12em" }}>••••••••</span> ·{" "}
                <span style={{ color: C.green }}>match</span>
              </span>
              <span style={{ color: settled ? C.green : C.grayLight, fontWeight: 500 }}>
                {settled ? "✓ resolved locally · in transit" : "resolving…"}
              </span>
            </div>
          </div>
        </div>
      ...
      {frame >= 196 && frame < 292 && (
        ...
        >
          <div style={{ width: 900 }}>
            <div style={{ fontFamily: FONT, fontSize: 24, fontWeight: 500, color: C.gray, marginBottom: 12 }}>Email</div>
            <div
              style={{
                background: C.white,
            ...
            <div style={{ fontFamily: FONT, fontSize: 24, color: C.gray, marginTop: 18, textAlign: "center" }}>
              typed by <span style={{ color: C.ink, fontWeight: 500 }}>your machine</span>, not by the model
            </div>
          </div>
        </div>
      )}
          ...
          >
            {[
              { label: "WHAT HOLO3 SAW", content: <Chip token={TOKEN} size={34} animated={false} />, edge: C.ink },
              {
                ...
                content: <span style={{ fontFamily: MONO, fontSize: 34, fontWeight: 500, color: C.green }}>{REAL}</span>,
                edge: C.green,
              },
            ].map((side, i) => (
              <div
